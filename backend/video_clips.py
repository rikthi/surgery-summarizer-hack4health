...
def generate_phase_clips(video_path: Path, segments: Sequence[Dict[str, Any]], job_id: str) -> List[Dict[str, Any]]:
    ...
    for index, segment in enumerate(segments):
        parts = segment.get("segments") or [segment]
        intervals: List[Tuple[float, float]] = []
        total_duration = 0.0
        for part in parts:
            part_start = max(float(part.get("start_second", 0.0)), 0.0)
            part_end = max(float(part.get("end_second", part_start)), part_start)
            intervals.append((part_start, part_end))
            total_duration += humanize_duration(part_start, part_end)
        ...
        success = False
        if FFMPEG_PATH:
            success = _extract_phase_with_ffmpeg(video_path, intervals, clip_path)
        if not success:
            success = _extract_sequence_with_cv2(video_path, intervals, fps, clip_path)
...
def _extract_phase_with_ffmpeg(
    video_path: Path,
    ...
    try:
        for idx, (start, end) in enumerate(intervals):
            duration = humanize_duration(start, end)
            part_path = destination.with_name(f"{destination.stem}_part{idx:02d}{destination.suffix}")
            if not _extract_with_ffmpeg(video_path, start, duration, part_path):
                return False
        ...
        if len(temp_files) == 1:
            temp_files[0].replace(destination)
            return destination.exists() and destination.stat().st_size > 0
...
def _extract_sequence_with_cv2(
    video_path: Path,
    intervals: Sequence[Tuple[float, float]],
    fps: float,
    destination: Path,
